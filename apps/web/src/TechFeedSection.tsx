...
import { createTechFeedClient, FEED_CATEGORIES, FEED_INTERESTS, mergeFeedPage, safeFeedUrl, summaryLabel } from './techFeed.mjs';
import type { FeedArticle, FeedPage, FeedPreview, FeedState } from './techFeedTypes';
import './techFeed.css';
...
export function FeedArticleCard({article,onSave,onPlan,busy,timeZone}:{article:FeedArticle;onSave:()=>void;onPlan:()=>void;busy:boolean;timeZone:string}) {
  ...
  return <article className="feed-card">
    ...
    <h3>{article.title}</h3>
    <p className="feed-sources">{article.sources.map(source=>source.name).join(' · ')}</p>
...
export default function TechFeed({supabase,userId,timeZone,onPlan,linkedTodo=null}:Props) {
  ...
  useEffect(()=>{const controller=new AbortController();lifetime.current=controller;return()=>controller.abort();},[userId]);
  useEffect(()=>{
  ...
  async function action(key:string, work:()=>Promise<void>) {
    ...
    try {await work();}
    catch(e){if(!lifetime.current?.signal.aborted)setError(e instanceof Error?e.message:'요청을 처리하지 못했어요.');}
    finally {actionLock.current=false;if(!lifetime.current?.signal.aborted)setBusy('');}
  }
  ...
  return <section className="tech-feed" aria-labelledby="tech-feed-title">
    ...
    {state?.enabled && <>
      ...
      <details className="feed-subscriptions"><summary><Settings2 size={17}/>구독·관심 분야 관리 <span>{state.sources.filter(item=>item.subscribed).length}개 구독</span></summary>
      ...
      <p className="feed-sync">매시간 새 글 수집 · {state.last_success_at ? `마지막 성공 ${new Intl.DateTimeFormat('ko-KR',{timeZone,dateStyle:'short',timeStyle:'short'}).format(new Date(state.last_success_at))}` : '첫 수집 대기'}{failures.length>0&&` · ${failures.length}개 소스 재시도 대기`}</p>
    ...
    {cursor && !loading && <button className="secondary feed-more" disabled={Boolean(busy)} type="button" onClick={()=>void loadMore()}>{busy==='more'?'불러오는 중…':'이전 소식 더 보기'}</button>}
    </>}
  </section>;
}
