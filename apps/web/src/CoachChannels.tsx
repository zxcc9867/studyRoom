...
export default function CoachChannels({supabase,userId,settings,request,onManage}:{supabase:SupabaseClient;userId:string;settings:CoachSettings;request:CoachRequest;onManage:()=>void}) {
  ...
  async function run(task:()=>Promise<void>){if(lock.current)return;lock.current=true;setBusy(true);setError('');setMessage('');try{await task();if(live.current){await load();setMessage('알림 설정을 반영했어요.');}}catch(failure){if(live.current)setError(failure instanceof Error?failure.message:'알림 설정을 처리하지 못했어요.');}finally{lock.current=false;if(live.current)setBusy(false);}}
