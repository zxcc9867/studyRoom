...
render();
`;
async function withMounted(run){
 ...
 const server=createServer((request,response)=>{response.setHeader('Content-Type',request.url==='/app.js'?'text/javascript':'text/html');response.end(request.url==='/app.js'?built.outputFiles[0].text:'<!doctype html><html><body><div id="root"></div><script src="/app.js"></script></body></html>');});
...
browserTest('mounted reactivation does not abort active generation and revalidates once after completion',async()=>withMounted(async page=>{
 ...
 await page.getByRole('button',{name:'오늘 요약 다시 보기',exact:true}).click();
