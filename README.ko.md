...
수집 설정과 읽기 필터를 구분합니다. 자유롭게 입력한 관심 내용은 **무엇을 수집할지** 정하고, 접힌 주제·출처 필터는 **이미 모아둔 글에서 무엇을 볼지** 좁힙니다. 최신·저장 모두 현재 페이지가 아닌 전체 글을 기준으로 필터 항목을 제공하며, 긴 관심 문장 대신 실제 내용과 맞는 짧은 태그를 표시합니다.

글은 AI 분류를 우선하고, 없으면 명확한 근거가 있을 때만 `기술 소식 / 실무·튜토리얼 / 사례·심층 분석`으로 보완합니다. 억지로 분류하지 않으며 카드마다 미분류 문구를 반복하지 않습니다. 소개는 깔끔한 미리 보기로, 펼친 본문은 제목·목록·코드 구조로 읽을 수 있고 외부 HTML은 실행하지 않습니다.

`오늘 수집된 내 피드`는 프로필 시간대와 글의 최초 수집 시각을 기준으로 하루 전체 통계를 자동 표시합니다. 발행일·현재 페이지·필터와는 별개입니다. AI 인사이트는 버튼을 눌렀을 때만 권한이 확인된 소개 최대24개를 분석하고 원문 근거를 연결합니다. 충분한 소개가 최소2개 필요하며 영상 내용을 분석하지 않습니다. 캐시는 재사용하고 새 글이 생기면 갱신 필요를 표시하며, 출처 접근 권한을 잃으면 관련 결과를 숨깁니다. 기존 사용자별 하루 실제 AI6회 한도를 공유하고 실패도 계산하며 유료 전환은 없습니다. [설계와 제한](docs/tech-feed/daily-briefing-design.md) · [현재 반영 상태](memory-bank/progress.md).
