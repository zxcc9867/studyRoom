...
웹에서 공개 기술에 관한 관심 내용을 입력하고 소식을 받거나, 내용을 바꾸거나, 수신을 잠시 중지합니다. 일반 사용자에게 검색 API 키나 SNS 연동을 요구하지 않습니다. 새 기술 발견 → 한국어 요약 또는 출처·검색 소개 → 원문 읽기 → 공부할 일 추가로 이어집니다. 최신/저장, 필터, 20개씩 더 보기를 제공하며 구독 출처는 고급 설정에 둡니다. 첫 화면은 계속 `오늘`이고 설정·저장·할 일 연결은 계정별로 서버에 보존합니다.
