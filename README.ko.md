...
## 독서실 2.0 — 관심 내용으로 받아보는 기술 피드

원문이 제공하는 썸네일을 피드 카드에서 보여주며 다이어그램이 잘리지 않도록 비율을 유지합니다. 첨부된 YouTube/Vimeo 영상은 버튼을 누를 때만 불러오고 자동 재생하지 않으며 닫을 수 있습니다. 미디어가 없거나 차단·오류가 나면 글과 출처 링크를 유지합니다. 기존 글도 수동/정기 작업당 최대3개씩 공용 캐시에 순차 반영하므로 모든 글에 사진·영상이 보장되지는 않습니다. 무관한 이미지 검색, 전체 본문 저장, 추가 AI/유료 API는 사용하지 않습니다.

특정 업체나 도구에 한정하지 않고 **사용자가 설정한 관심 기술의 기술 블로그·실무 사례·해설·활용 가이드**를 찾습니다. AWS나 Claude Code는 예시일 뿐입니다. 같은 검색1회 예산 안에서 의도를 순환하고 최근1년까지 탐색합니다. 새 검색의 영상·목록 페이지와 명확한 홍보·시간표 문구를 걸러내며, 한글 웹폰트·큰 본문과 제목/소개 옆 출처 링크로 읽기 쉽게 제공합니다. 제공된 소개를 번역하며 전체 글은 원문 사이트에서 읽습니다.


웹에서 공개 기술에 관한 관심 내용을 입력하고 소식을 받거나, 내용을 바꾸거나, 수신을 잠시 중지합니다. 일반 사용자에게 검색 API 키나 SNS 연동을 요구하지 않습니다. 새 기술 발견 → 한국어 요약 또는 출처·검색 소개 → 원문 읽기 → 공부할 일 추가로 이어집니다. 최신/저장·필터와 20개 단위 페이지 전환(이전/다음·방문한 페이지 번호)을 제공합니다. Threads형 한 열 피드에서 출처·주제·한국어 소개를 읽고, 긴 내용·AI 요약·원문은 펼쳐 확인합니다. 관심 설정과 수집 출처는 접힌 패널에 정리합니다. 첫 화면은 계속 `오늘`이고 설정·저장·할 일 연결은 계정별로 서버에 보존합니다.

출석과 분리된 수집기가 승인된 RSS·Atom·Hacker News와 공개 웹 검색을 함께 처리합니다. 정규화 후 동일한 관심 내용은 한 시간 동안 결과를 공유합니다. 검색은 앱 전체 공용 월 최대 900회 시도로 제한하며 호출 전 무료 계정·한도를 확인하고 유료 서비스로 전환하지 않습니다. 검색 한도 소진 시 승인된 활성 RSS/API 수집은 유지합니다. AI는 실제 발췌·검색 소개만 요약하고, 기존 사용자별 하루 실제 6회 제한을 공유하며 실패도 계산합니다. 근거가 부족하면 소개와 원문 링크만 제공합니다.
`새 글 확인`을 누르면 완료 직후에도 즉시 다시 수집합니다. 실행 중 작업은 공유하지만5분 대기는 없습니다. 정기 수집은1시간 기준이며 여러 관심사는 한 번에 하나씩 순환 검색합니다. 요청당 검색1회와 수집 가능한 RSS/API 최대4곳을 기존 무료 한도 안에서 확인합니다. 매번 새 글이 있거나 AI 요약이 바로 완성된다고 보장하지 않습니다.


제목·소개는 DeepL API Free로 한국어 번역하며 AI 요약/코칭의 하루6회와 별도로 처리합니다. 번역 성공 결과는 공유하고 원문은 접어서 확인할 수 있습니다. 앱 공용 UTC월45만자 상한과 공급자의 실제 무료 잔여량을 함께 확인하며 유료로 전환하지 않습니다. 키 미등록·한도·실패 시 원문을 유지합니다. 운영자만 무료 번역 키를 등록하면 되며 일반 사용자는 키가 필요 없습니다. [번역 설정·검증 안내](docs/tech-feed/korean-translation.md).

웹 검색은 검색 제공자가 색인한 공개 결과를 대상으로 하며 모든 페이지나 비공개 SNS를 수집하지 않습니다. 검색 소개와 AI 요약을 구분합니다. 추천 RSS/API 8개는 **이용 조건 검토 대기**로 시작하고 공개 HTTPS 피드 추가는 미리 보기 후 최대 10개를 지원합니다. 검색 결과에 나왔다는 이유로 해당 출처의 RSS 재가공을 승인하지 않습니다.

**웹 사용자 설정과 운영 가동은 별개입니다.** 앱 운영자가 전용 무료 검색 키·종량제 비활성화·수집기 설정과 실제 수집을 확인해야 합니다. 키 미등록·한도 소진 시 검색을 멈추며 가짜 소식을 만들지 않습니다. [현재 반영 상태](memory-bank/progress.md) · [웹 검색 운영 안내](docs/tech-feed/search-provider.md) · [요구사항](memory-bank/prd-tech-feed.md) · [커리어 코드 복원](archive/career-coach/README.md). 커리어 전용 화면·자동 실행은 보관하며 시간대 설정과 공부 재시작 코칭은 유지합니다.
...
15. 내 페이지에서 주간·월간과 과거 기간을 선택해 공부 시간·출석·할 일·회고를 비교합니다. 선택 기간에 같은 방해가 2회 이상 반복되면 다음 세션 전 바꿀 환경 한 가지를 안내합니다. 설정의 적응형 알림은 최근 28일 공부 시작 패턴을 반영합니다.
...
- 주간·월간 학습 리포트: 현재와 과거 주·월을 선택해 완료 공부 시간, 할 일 완료율, 출석, 집중·에너지, 다음 행동을 비교. 진행 중인 기간은 오늘까지 같은 경과 일수, 완료 기간은 전체 기간을 비교하며 양쪽 날짜·일수와 월 일평균을 표시
- 정확한 리포트 로딩: 현재 계정의 저장된 시간대를 먼저 확인하고 선택 기간 기록을 페이지 조회. 로딩·오류를 0분으로 표시하지 않으며 실패 시 재시도. 기존 다음 행동의 계획 연결은 유지
- 리포트는 저장된 기록으로 다시 계산하며 자동 발송이나 고정 스냅샷은 아님. [상세 PRD](memory-bank/prd-study-reports.md)
- 반복 방해 조정: 선택한 리포트 기간의 완료 세션 회고에서 같은 방해가 2회 이상 반복될 때만 휴대폰·소음·피로·일정·기타 중 가장 빈번하고 최근인 한 가지를 골라, 새 버튼 없이 다음 시작 전 환경 조정을 안내
...
<p align="center">
  <img src="docs/images/study-forest-growth-path.png" alt="저폴리 섬, 씨드 나무, 5/7 반딧불 진행도와 성장 로드맵이 보이는 공부의 숲" width="100%" />
</p>

공부의 숲은 단순한 출석 배지가 아닙니다. 캐릭터가 섬을 직접 걷고, 물을 통과하지 않는 다리를 건너며, 집 안으로 들어가 공부 공간을 둘러봅니다. 출석과 작은 시작이 쌓일수록 섬도 함께 달라집니다.

| 살아 있는 섬을 걷기 | 나만의 공부 오두막으로 들어가기 |
| --- | --- |
| ![다리 위에 서 있는 캐릭터와 저폴리 섬](docs/images/study-forest-live-island.png) | ![Cozy Study Cottage 내부를 걷는 캐릭터](docs/images/study-forest-cottage-interior.png) |
| **의도대로 이동합니다.** 키보드·터치·클릭·자동 산책은 모두 육지와 다리의 걸을 수 있는 경로를 따릅니다. | **보상이 머무는 공간입니다.** 실제 문을 통해 들어가 가구가 놓인 실내를 걷고, 아래쪽 출구로 다시 섬에 나갈 수 있습니다. |

**성장은 섬에 남습니다.** 5/7 작은 시작은 나무 둘레의 씨앗 조명을 밝히고, 7일 연속 출석은 나무를 완성합니다. 완성한 나무 수에 따라 새 가구와 야외 보상이 해금됩니다.

<p align="center">
  <img src="docs/images/study-forest-atelier.png" alt="섬 테마, 집 포인트 색상, 야외 보상과 잠금 아이템을 보여주는 내 숲 꾸미기 화면" width="100%" />
</p>
...
- 누적 결석은 모든 기기에서 하나의 회복 루틴으로 정리하며, 해당 기간·결석 일수를 표시하고 보충 과제는 사용자의 현지 오늘 날짜에 생성
...
- 반복 방해 조정은 리포트가 조회한 선택 기간의 완료 세션·회고로 계산합니다. 1회·`none`·알 수 없는 값은 무시하고, 조정 안내 자체는 추가 query·스키마·RPC·서버 쓰기를 만들지 않습니다.
...
아래 설명과 화면은 이전 구현의 기록이며 이번 버전의 활성 기능이 아닙니다. 전용 코드·테스트는 `archive/career-coach/`에 보관하고 기존 DB 이력은 유지합니다.

하나의 커리어 목표와 편집 가능한 스킬 로드맵, 일정, 학습 기록을 연결해 오늘의 공부를 자동 추천합니다. 추천은 사용자가 수락한 경우에만 실제 할 일로 등록합니다.

- 서울·도쿄 등 시간대를 직접 선택하며 기기 변경이나 알림 설정 저장으로 선택이 덮어써지지 않습니다.
- 공부 가능 시간·휴식·생활 일정을 설정하고 선택한 Google 캘린더를 읽기 전용으로 연결합니다. 일정 제목은 AI에 보내지 않습니다.
- 추천의 관련 스킬·소요 시간·완료 기준을 확인하고 수락·시간 변경·건너뛰기를 선택합니다.
- Slack·Web Push·이메일은 연결하고 활성화한 채널에만 보냅니다. 연결하지 않으면 앱에서만 확인하며 이메일로 자동 우회하지 않습니다.
- 선택한 공개·비공개 GitHub 저장소의 개선 과제를 파일·커밋 근거와 함께 확인합니다. 비공개 코드의 AI 처리는 저장소별 선택이며 코드 실행·수정·PR 생성은 하지 않습니다.

외부 서비스의 앱 등록, 사용자 연결, 기기 알림 수신 검증은 서로 다른 단계입니다. [연결·운영 설정](docs/studyroom-v2-setup.md)과 [요구사항·출시 기준](memory-bank/prd-studyroom-v2.md)을 확인하세요.

## AI 서비스와 모델

| 항목 | 동작 |
| --- | --- |
| 서비스 | 서버 전용 OpenRouter |
| 설정 모델 | `OPENROUTER_MODEL`, 기본 `openrouter/free` 또는 명시적으로 지정한 `:free` 모델 |
| 실제 모델 | 성공 응답에서 받은 모델 ID를 결과에 기록. 라우터 이름과 구분 |
| 비용 정책 | 무료 모델·가격 0 제한, 유료 자동 전환 없음 |
| 기본 한도 | 출력 1,024토큰·20초, 환경변수로 선택 설정 |
| 파일럿 공통 호출량 | 사용자별 하루 실제 호출 최대 6회, 실패 포함. 캐시는 호출하지 않음 |
| AI 실패 | 확정한 로드맵과 규칙 기반 추천을 제공하고 AI 생성 결과로 표시하지 않음 |

동적 라우터는 고정 모델이나 항상 최고 성능을 보장하지 않습니다. 키·개인 프롬프트·비공개 소스 코드는 문서에 기록하지 않습니다.

## 실제 구현 화면

구현한 React 화면을 테스트 데이터로 실행해 촬영했습니다. 실제 Google·GitHub 계정 연결이나 알림 수신을 입증하는 화면은 아닙니다.

![자동 오늘의 추천과 근거·완료 기준](docs/images/studyroom-v2-today.png)

![편집 가능한 커리어와 스킬 로드맵](docs/images/studyroom-v2-career.png)

![공부 가능 시간과 채널별 알림 선택](docs/images/studyroom-v2-settings.png)

![390px 모바일 브라우저 폭에서의 설정 화면](docs/images/studyroom-v2-mobile.png)

![생활 일정 편집과 저장된 시간대](docs/images/studyroom-v2-calendar.png)
