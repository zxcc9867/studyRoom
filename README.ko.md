...
새 기술 발견 → 한국어 AI 요약 또는 출처 소개 → 원문 읽기 → 공부할 일 추가로 이어집니다. `기술 피드`에서 최신/저장, 관심 분야·출처 필터, 20개씩 더 보기, 접힌 구독 관리를 제공합니다. 첫 화면은 계속 `오늘`이며 저장·구독·할 일 연결은 계정별로 서버에 보존합니다.

출석 Cron과 분리한 수집기가 승인된 공개 RSS·Atom과 Hacker News API를 매시간 확인합니다. 무료 AI만 사용하고 기존 사용자별 하루 실제 6회 제한을 공유하며 실패도 계산합니다. 모든 글의 요약은 보장하지 않습니다. 추천 소스 8개는 **이용 조건 검토 대기**로 등록하고 공개 HTTPS RSS 추가는 미리 보기 후 최대 10개를 지원합니다.

**코드는 운영에 배포했으며 기술 피드는 아직 비활성입니다.** DB·서버 함수·웹 배포와 비활성 Cron 등록을 완료했습니다. 이용 조건 승인, 무료 AI 설정과 파일럿·운영 검증 후 활성화합니다. [배포 기록](docs/tech-feed/deployment-20260912.md) · [요구사항](memory-bank/prd-tech-feed.md) · [출시 안내](docs/tech-feed/release.md) · [커리어 코드 복원](archive/career-coach/README.md). 커리어 전용 화면·자동 실행은 보관하며 시간대 설정과 공부 재시작 코칭은 유지합니다.
