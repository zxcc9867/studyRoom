...
## 21. 2026-06-15 Update: Camera Status Diagnosis UI

### Decision

- 카메라 상태 진단은 브라우저 안에서 이미 계산 중인 상태만 사용한다.
- 사진, 영상, 프레임, 포즈 랜드마크 원본은 계속 저장하거나 서버로 보내지 않는다.
- `cameraDiagnostics.mjs`는 브라우저 보안 지원, 권한 차단, 스트림 트랙 상태, 프레임 상태, 검은 화면, 상반신 미감지, 자동 일시정지, 정상 상태를 사용자 문구와 체크리스트로 변환한다.
- `Today Focus` 카메라 카드 안에 별도 모달이 아닌 얇은 진단 스트립을 표시한다.
- 진단 UI는 문제 원인과 다음 조치를 설명할 뿐, Slack 경고, 출석 상태, 공부 시간 계산, Supabase 데이터 구조를 변경하지 않는다.

### Added Functional Requirements

- [x] HTTPS 또는 localhost가 아닌 환경에서는 안전한 주소가 필요하다고 안내한다.
- [x] 브라우저/운영체제 카메라 권한 차단 시 주소창 권한과 OS 개인정보 보호 설정을 확인하라고 안내한다.
- [x] 카메라 트랙이 없거나 종료/음소거/비활성 상태면 연결 끊김 또는 영상 중단으로 안내한다.
- [x] current frame 또는 video size가 아직 없으면 영상 연결 확인 중 상태로 표시한다.
- [x] 거의 검은 프레임이면 사용자 부재가 아니라 카메라 영상 문제로 안내한다.
- [x] 영상은 정상이나 상반신이 보이지 않으면 자리 비움/상반신 미감지로 안내한다.
- [x] 정상 감시 중에는 카메라 정상 상태와 이후 자리 비움 경고 기준을 간단히 표시한다.

## 22. 2026-07-17 Update: Camera Start Cancellation and Timeout

### Decision

- `getUserMedia()` 권한/장치 요청이 응답 없이 대기하면 UI를 무기한 `starting` 상태로 두지 않는다.
- 카메라 스트림 시작 제한은 15초이며, 초과 시 요청을 오류 상태로 끝내고 권한 확인 후 재시도하라는 안내를 표시한다.
- 각 시작 요청에 증가하는 attempt 번호를 부여한다. 세션 종료나 카메라 중지는 번호를 증가시켜 이전 비동기 결과를 무효화한다.
- 무효화 또는 timeout 뒤 늦게 도착한 MediaStream은 모든 track을 즉시 중지하고, 늦게 생성된 상반신 detector도 닫는다.
- 활성 세션이 없을 때 카메라 버튼을 disabled로 두지 않는다. 클릭은 스트림을 시작하지 않고 먼저 공부 세션을 시작해야 한다는 안내를 보여준다.

### Added Functional Requirements

- [x] 카메라 시작 요청은 15초 안에 성공 또는 사용자 재시도 가능한 오류 상태로 종료한다.
- [x] 세션 종료는 진행 중 카메라 시작 attempt를 무효화하고 `idle` 상태로 정리한다.
- [x] 취소된 attempt의 늦은 성공/실패가 현재 UI 상태를 덮어쓰지 않는다.
- [x] timeout 이후 늦게 도착한 스트림은 track 누수를 남기지 않는다.
- [x] 세션이 없을 때 카메라 켜기 버튼 클릭은 무반응이 아니라 사용 조건 안내를 표시한다.
- [x] 시작 성공, timeout, 늦은 스트림 정리, 앱 연결을 회귀 테스트로 검증한다.
