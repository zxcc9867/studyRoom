...
- 5분 동안 얼굴이 감지되지 않으면 공부 타이머를 자동 일시정지하고 해당 미감지 시간은 공부 시간에서 제외한다.
- 10분 동안 얼굴이 감지되지 않으면 공부 세션을 자동 종료한다.
...
8. 얼굴이 다시 감지되면 제외 시간을 누적하고 타이머를 다시 진행한다.
9. 얼굴이 10분 동안 감지되지 않으면 앱이 `end_study_session` RPC를 자동 호출하고 제외 초를 함께 전달한다.
...
- 5분 미만의 짧은 얼굴 미감지는 공부 시간에서 제외하지 않는다.
...
- 얼굴이 5분 이상 감지되지 않으면 현재 미감지 구간 전체를 공부 시간에서 제외한다.
- 제외 시간이 발생하면 UI는 현재 세션을 `자동 일시정지` 상태로 표시한다.
- 얼굴이 다시 감지되면 제외 시간을 누적하고 타이머를 다시 진행한다.
- 얼굴이 10분 이상 감지되지 않으면 앱이 세션을 자동 종료한다.
- 자동 종료와 수동 종료, 페이지 이탈 종료는 모두 `p_excluded_seconds`를 `end_study_session` RPC에 전달한다.

### Added Functional Requirements

- [x] 5분 이상 얼굴 미감지 시 현재 세션/오늘 공부 시간 표시에서 미감지 시간을 제외한다.
- [x] 얼굴 복귀 시 제외 시간을 누적하고 세션 타이머를 재개한다.
- [x] 10분 이상 얼굴 미감지 시 세션을 자동 종료한다.
