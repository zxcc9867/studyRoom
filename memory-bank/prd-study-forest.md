...
## 14. 2026-07-17 Update: Side Handrails and Physics-aware Bridge Corridor

### Decision

- 다리 전체를 90도 회전하는 것만으로는 충분하지 않다. 난간 beam은 판자가 이어지는 로컬 X축과 평행하고, 로컬 Z축 양옆에 배치해 입구와 출구를 열어 둔다.
- `forestBridgePhysics`가 판자 길이/폭, 난간 위치/두께, 기둥 반경, 캐릭터 반경, 정규화 통로 좌표를 함께 소유한다.
- 캐릭터 반경은 실제 야외 모델의 몸통·머리·그림자 최대 폭에 맞춰 0.48 world unit으로 둔다.
- 데크 외곽은 정규화 X 47–64, 실제 캐릭터 중심이 통과할 안전 통로는 난간 기둥 반경과 캐릭터 반경을 제외해 계산한 X 52–58이다.
- 데크 위지만 안전 통로 밖인 위치는 `bridge-rail` 충돌로 처리하고 중앙 통로로 이동하라는 안내를 표시한다.
- 정적인 저폴리 장면에 별도 rigid-body 라이브러리를 추가하지 않는다. 렌더링과 이동 판정이 같은 치수 모델을 사용하는 결정적 collider가 모바일 성능과 현재 요구에 더 적합하다.

### Added Functional Requirements

- [x] 다리 양 끝은 난간 mesh로 막히지 않는다.
- [x] 두 난간 beam은 다리 진행 방향과 평행하게 양옆에 배치된다.
- [x] 난간 기둥은 진행 방향을 따라 배치되고 중앙 통로를 가로막지 않는다.
- [x] 캐릭터 중심은 X 52–58에서 물 위 다리를 건널 수 있다.
