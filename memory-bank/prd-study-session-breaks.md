...
[실제 공부와 계획 PRD](prd-actual-study-plan.md)에 따라 웹 재개 시 할 일별 기존 누적 공부량을 유지하고 남은 분량의 조정 일정을 제안한다. 충돌 시 전체 변경안 수락 후에만 세션과 일정을 확정한다. 휴식/lease/카메라 제외/기존 Expo API 정책은 유지하며 재개를 새로운 최초 시작 지연으로 평가하지 않는다.
