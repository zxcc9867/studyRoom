## 2026-09-12 개정: 커리어에서 독립한 시간대 저장

- 프로필 시간대는 커리어 설정/자동 실행 없이 저장할 수 있어야 한다. 기술 피드 파일럿 비활성 계정에도 인증된 `tech-feed`의 `timezone` action을 제공한다.
- 서버는 토큰 소유자와 유효 IANA 시간대를 검증하고 그 사용자의 `profiles.time_zone`만 수정한다. 익명 사용자는 허용하지 않는다.
- 웹은 확인한 계정 토큰을 요청에 고정하며 계정 전환 후 늦게 도착한 결과를 폐기한다.
- 기존 출석/타이머/과거 기록/알림 설정은 변경하지 않는다. 별도 기기에서 저장된 시간대를 읽는 기존 흐름을 유지한다.
- 관련 문서: `prd-tech-feed.md`, `docs/tech-feed/release.md` (운영 미적용).
...
## 2026-09-10 Update: Weekly and Monthly Study Reports

- 사용자는 내 페이지에서 주간/월간과 과거 기간을 선택하고 이전 기간과 비교한다.
- 진행 중인 기간은 오늘까지, 완료된 기간은 전체 기간을 비교하며 월별 일수 차이와 일평균을 표시한다.
- 현재 계정의 저장된 시간대가 준비되고 필요한 기록 조회가 성공하기 전에는 숫자를 확정해서 보여주지 않는다. 실패 시 재시도한다.
- 기존 다음 행동·방해 조정 안내는 선택한 리포트 기간에 적용한다. 이전의 현재 주/이미 로드된 데이터 한정 설명은 새 리포트 조회 계약으로 대체한다.
- 서버 쓰기·AI 비용·새 알림·Expo 리포트 화면은 추가하지 않는다. 상세: prd-study-reports.md.
