...
- 읽기 전용 확인: 운영 Supabase bqohkdzvxbrokkmuhysx에서 매시간 Cron 활성, 9월 19~23일 신규 기사 저장 확인(23일 9건), 24일 자정 직후 0건, 검색 예산 145/900, 추천 RSS 8곳 승인 대기.
- 최근 정시 worker HTTP 503/타임아웃, 21~23일 tech_feed_runs 완료 0·running 누적. 24일 00:17 JST 수동 요청은 주제 검색 성공이지만 최종 결과 null/lease 만료. 정기·수동 수집의 완료 상태는 정상으로 볼 수 없다.
- 최근 발견 27건은 모두 오래된 원문 발행일을 갖고 목록은 원문 발행일 우선 정렬한다. 첫 페이지 20건에는 19일 이후 발견한 글이 0건이어서 새 글이 없는 듯 보인다.
- 후속 계정 검증: auth.users 2개, 수신 계정 1개. 유일한 수신 계정의 목록 RPC는 총 142건·첫 표시일 9월 18일, 19~23일 발견한 가시 글 27건은 37~138번째로 밀린다. 첫 20건에는 0건.
- 변경: 진단 기록만 갱신. 코드·운영 설정·데이터 변경, 커밋·푸시·배포 없음. 실계정 브라우저 조작과 Edge 내부 예외 로그는 미검증.

### 2026-09-23 — 기술 피드 PC 공백·정보 위계 개선

- 완료: 피드에 한정해 작업 영역을 확장하고 최대 1800px 중앙 캔버스, 넓은 PC 2열 기사/모바일 1열을 구성했다. 오늘 브리핑에는 수집·출처·주요 주제를 요약하고 상세 통계와 인사이트를 접기 영역으로 이동했다.
- 변경: `apps/web/src/main.tsx`, `apps/web/src/techFeed.css`, `apps/web/src/FeedDailyBriefing.tsx`, 관련 브리핑/브라우저 테스트. 서버·DB·다른 화면은 변경하지 않았다.
- 검증: 신규 브리핑 테스트와 Chromium 375/1440/2200px 레이아웃 테스트가 변경 전 실패한 뒤 통과. 전체 801/801(skip 0), build, mobile:check, docs:check, Edge 11/11. lint 스크립트 없음.
- 배포: 사용자 명시 승인 후 d25f91b를 main에 푸시했다. GitHub Actions 35877923111 전체 단계 성공, Vercel production dpl_DijdUWT2zBjUN7im5K7WKopKU1Sf READY 및 커밋 일치, https://study-room-attendance.vercel.app/ HTTP 200 확인. 합성 데이터 스크린샷은 `output/playwright/feed-after-*.png`(Git 제외).

### 2026-09-23 — 이번 세션 할 일 카드 가독성
...
- 운영 배포: 코드 34a2f34(main), Actions 35825343580 success, Vercel dpl_DG9uPtM6cq3gHbJCHD733QgVF8C8 READY. https://study-room-attendance.vercel.app HTTP 200 및 운영 TechFeedSection JS HTTP 200/한국어·영어 원문 문구 확인. 두 Edge 함수 무인증 HTTP 401.
- 운영 조사: RSS 추천 8곳은 모두 이용 조건 확인 대기, 원문 한글 제목 19/135건. 추가 검색은 결과 언어를 보장하지 않는다.
...
## 2026-09-14 — 회복루틴 대기/재시도 수정 검증

- apps/web/src/dashboardData.ts: bounded request, batch/페이지 fetch abort 지원. main.tsx: 조회 전용 상태/최신요청 가드/초기 오류 표시/회복 저장 직후 시작 잠금 해제/불확실한 저장 재확인.
- 신규 dashboardRequest.test.mjs 4개 실패 재현 후 통과. goalHistoryLoading 테스트 대역에 실제 SDK abortSignal 인터페이스 반영. 기존 단일 시작 버튼 가이드 회귀 유지.
- 전체624개, 웹빌드, mobile:check, docs:check 통과. 로컬 API 가상 응답 브라우저390/1440에서 지연·재시도·저장 후 활성·카메라 진입 확인. 실제 사용자 데이터/세션 변경 없음.
- 다음: 독립 리뷰 완료 및 production 배포 검증. 미리보기/가상계정 검증은 운영 사용자 E2E 성공을 대신하지 않음.


## 2026-09-14 — 회복루틴 이후 시작 비활성화 읽기 진단

- main.tsx 제출→submitted 반영→전체 재조회 busy 잠금, dashboardData.ts 무제한 요청 대기 경로 확인.
- 운영 MCP SQL 2회 연결 timeout, 공개키 Auth health 15초 timeout. 회복 제출 상태는 조회 실패로 미확인.
- 진단 문서만 로컬 기록. 제품 코드/DB/세션/배포 변경 없음. 서버 복구 후 상태 검증 및 승인 후 오류 복구 UI 보완 필요.


## 2026-09-14 — 로그인 인증 불가 진단

- 읽기 점검: 운영웹/Auth health/settings200, DB잠금0, 새브라우저 로그인화면정상/console0, Vercel최근오류0.
- 사용자 기존세션에 대한 문제는 재현/확정하지 못함. 오류문구·스크린샷 필요.
- 세션삭제/로그아웃/코드변경/재배포없음. 진단문서만 로컬갱신, 커밋·푸시하지 않음.
...
- Supabase tech-feed v17 및 worker v19 ACTIVE·JWT true 확인. DB/cron/RSS권한/비밀값/예산 변경 없음.
- 변경 코드·테스트·README3개 언어판과 memory-bank를 main에 반영해 기존 Actions 웹 배포 진행. 임시output/.playwright-cli 제외.


## 2026-09-14 — 기술 블로그 중심 피드 품질 개선 구현

### 완료한 작업

- 사용자 정정 반영: 예시3종 한정이 아니라 임의 관심 기술의 블로그·사례·가이드 순환검색. 최근1년과 실제 발행일, 원문 출처 보존.
- 영상/색인 신규 수집 필터와 명확한 홍보/시간표 정리. query permalink와 장애 타임라인·기술 문장 보존 경계 회귀 추가.
- 한글 Pretendard 웹폰트, PC17/모바일16px 본문, 보조13px 이상. 제목/소개 출처 원문 링크, 기존 영상은 안내와 링크 보존.
- README3개 언어판 및 PRD/설계/현재 맥락/오류 기록 갱신.

### 검증 방법

- 신규 RED→GREEN9개 회귀 포함 전체609/609·Edge8/8·웹빌드·모바일호환·README24개자산·diff검사 통과. 실제 검색 어댑터 HTML/문단 손실 검증 및 독립 최종 재검토 통과(Important3건 수정, 추가발견 없음).
- Chromium390/1440px: 글꼴로드 성공, 가로넘침0, 출처14px/높이44px. 페이지20개/캐시복귀/펼치기/저장경계/필터/오류재시도/계정전환 통과.
- 로컬 예시 데이터 화면이며 운영 로그인/실수집 결과 확인은 배포 후 별도 수행.

### 남은 작업

- 변경사항은 로컬. Git/운영 Edge2개/웹 배포는 별도 요청 후 실행. 기존 DB 스키마·RSS승인·출석Cron·유료설정·비밀값은 변경하지 않음.


## 2026-09-14 — 피드 품질 후속 요청 조사

- 실제 운영 검색 글에서 YouTube 방송 설명/시간표가 원문·한국어 소개에 포함됨 확인. 원문링크는하단에있지만 출처텍스트는링크없음, 본문모바일14px·보조10~11px.
- 뉴스 편향 원인: focusedSearchQuery의 technology news 접미어와 Tavily 최근1주 고정. 기본RSS8개 미승인 상태를 읽기로 확인.
- 다음: 짧은 개선안 승인 후 UI/검색품질 회귀→구현. 기존저장데이터 삭제·출처승인·유료호출·운영변경 없음. 현재진단기록만로컬저장.
...
## 2026-09-13 — SNS형 읽기 피드와 페이지 탐색

- 완료:20개 페이지 전환/방문 번호/캐시 복귀, 필터·계정 초기화, 빈 저장 페이지/조회 실패 보존. 출처·태그·한국어 내용·펼친 요약/원문·행동 버튼·접힌 설정 및 모바일 대응.
- 변경: TechFeedSection.tsx, techFeed.css, feedPresentation.mjs/.d.mts, 카드/페이지 단위 회귀와 브라우저 fixture/검증 도구, README3언어/관련memory-bank.
- 검증: 전체600/600, 웹 빌드, mobile:check, docs:check24이미지/3언어, git diff --check. Chromium390/1440px 및 실제 컴포넌트 행동 확인. 독립검토P2 두 건 수정·재검토 승인.
- 재현: node scripts/feed-ui-preview.mjs → playwright-cli -s=feed-ui open http://127.0.0.1:5184 → run-code --filename apps/web/test/fixtures/feed-ui-browser-check.js 및 feed-ui-edge-check.js.127.0.0.1 예시 API만 사용, 운영 연결 없음.
- 배포: 로컬 완료, 이전 번역 서버 적용 유지. 커밋·푸시·웹배포는 별도 승인 대기. 운영 계정 간 신규UI 검증 미실행.

## 2026-09-13 — 번역7건 완료, 웹 배포 남음

- 운영7건 번역ready, POST3/7259자, DB20260913140137, API v16/worker v18 ACTIVE JWTtrue. 실제 소스원문↔한국어 제목/소개 샘플 대조,원문일치 캐시7건 확인.
- 변경사항 커밋/푸시/main웹배포는 승인 시스템의 별도Git승인 요구로 중단. 명령 실행 전 거부, HEAD1d38625/index빈상태 확인. GitHub/Vercel에는 이번 번역UI 미반영, 로컬파일과 운영DB/함수는 유지.
- 다음: 명시적 커밋+main푸시+웹배포 승인 후 기존 workflow 성공/READY/HTTP200 확인. 테스트591·Edge8·웹/모바일/문서 및 독립검토 완료.

## 2026-09-13 — 번역 실운영 성공

- 일반Cron14:17UTC 실제 DeepL 번역3건 저장, provider ready, POST1회/3007자, 잔여4건순차처리. 브라우저를 닫아도 서버예약 작업이 번역함을 확인.
- 반환한도가50만자보다 큰 Free 응답 거부 버그를 max(0,min(500000,reported_limit)-count)로 수정. 안전상한은 늘리지 않음. 사전검사만 실패한7건 재시도 복구.
- 최종 전체591/591·Edge8/8·웹/모바일/README·독립 최종검토 통과. 웹배포와 전체캐시재사용 확인은 후속기록.

## 2026-09-13 — 번역 운영 배포 진행

- 사용자 명시 승인 후 DB20260913140137 적용, tech-feed/worker 서버 배포, 별도 Free 키 등록 확인. 새 RLS/EXECUTE 비노출 및 출석Cron 유지 확인.
- 실제Cron 사전확인 usage_invalid_limit 오류로 번역대기. 실제 POST·문자차감0. stage/http/validation 고정코드만 기록하며 키/응답본문은 노출하지 않는 진단 추가(RED→GREEN), 전체590 통과.
- 운영·로컬 스키마파일명 정합화. 웹 배포/실제 공급자 성공은 후속 최종 기록으로 확인한다.

## 2026-09-13 — 번역 최종 로컬 검증 및 배포 보류

- Node589/589·Edge8/8/10개진입점·웹 빌드·모바일·README 통과. 독립 리뷰 중요1건(취소→전역 한도 오인)을 수정 후 재검토 승인.
- reserve는 reserved/quota_exhausted/deferred를 구분한다. 수신중지/원문경쟁 취소는 차감과 전역 backoff 없이 lease를 반환하고 다른 구독자가 이어 처리한다. 상태 API 키 미등록/중지/한도 및 키 비노출 회귀 추가.
- 운영 DB 적용 요청은 승인 시스템이 별도 배포 승인 부족으로 거절. migration 목록에 새 번역 migration 없음 확인, 함수·웹 배포와 Git 커밋/푸시 미실행. 운영 상태는 이전 버전 유지.
- 다음: 명시적 DB·서버·웹 배포 승인과 운영 Free 키 등록. 실제 번역 품질/브라우저 확인은 그 후 가능. 상세 docs/tech-feed/korean-translation.md.

## 2026-09-13 — 한국어 피드 번역 로컬 구현

- 사용자 DeepL Free 구현 승인 후 제목·소개 자동 번역과 접힌 원문 보기, 서버 상태 표시 추가. 번역과 AI 요약/코칭 예산 분리.
- 공통 기사 캐시/원문 변경 무효화, 제공자 lease90초, 앱 UTC월450000자 원자 차감, 무료 사용량 사전 확인, 최대3개 batch, 오류 backoff/RLS 서버 전용 RPC 구현.
- 검증: Node587/587, Edge8/8 및10개entrypoint, 웹 빌드, 모바일 호환 통과. 운영 배포와 독립 리뷰 후 최종 기록 예정.
- 남은 외부 설정: 운영 DEEPL_API_KEY 없음. 실제 DeepL 번역·기술용어 품질 표본/브라우저 사용자 흐름은 미검증. 운영 수집·출석 기존 설정 유지.

## 2026-09-13 — 번역 공급자 비교 및 예산 분리 요구사항

- 번역에 코칭6회 한도를 적용하지 않는 원칙 사용자 승인, PRD 갱신. DeepL Free 문자기준과 기존 OpenRouter 무료 호출기준 공식자료 비교.
- 제안: 제목/소개 전용 번역API + AI 요약 분리. 공급자 선택/키 준비 및 기술글 표본 품질 검증 후 구현. 이번 변경은 로컬 문서뿐이며 외부가입·결제·연결·코드·배포 없음.

## 2026-09-13 — 한국어 번역 가능 여부 검토

- 현재 영어 표시 원인 확인: 원문 title/excerpt 직접 출력, 한국어 요약과 제목/소개 번역은 별도 기능. 사용자는 수집글 표시를 확인했다고 보고함.
- 기존 무료 AI/공유 캐시를 이용한 제목·소개 한국어 기본 표시를 제안. 한도 도달 시 모든 글의 즉시 번역은 보장하지 않는다. 설계 확인 대기이며 이번 턴은 조사 기록만 로컬 갱신.
...
- 검증: Node533/533, Edge10진입점·8/8, 웹 빌드·모바일·README 재검증, 신규 RLS5개 및 피드 RPC23개 직접 호출 권한 차단 확인.
- 변경: 검토된42파일과 배포 기록. 신규 SQL 최초 로컬 번호20260912122630을 실제 적용 번호20260912150427로 변경(SQL 동일).
- 남음: main 푸시/웹 READY 확인. 키·worker 인증·소스 승인/실제 수집 가동은 별도. 상세 docs/tech-feed/deployment-20260913.md.

## 2026-09-12 — 공개 웹 검색 기술 피드 로컬 완료 / 운영 반영 대기

- 완료: 관심 설정·수신 관리, 정규화 주제 공유1시간 캐시, 앱 공용 월900회 검색 시도 상한, 무료 확인 실패/한도 소진 시 검색만 중지, RSS/API 독립 처리, 실제 소개 근거만 무료 AI 요약, owner별 설정·저장/할 일 연결.
- 검토: 서버/UI 작업별 수정 완료 후 전체42파일 최종 검토. 마지막 문서3개 발견 사항(EOF/옛 설계/시각 설명)을 단일 wave로 수정하고 재검토 승인받았다. 중요 미해결 없음, punycode 경고만 비차단 후속 관리.
- 최종 검증: Node533/533, Edge10개 타입·8/8, 웹 타입/빌드, 모바일 호환성/타입, README24참조, staged diff 검사 통과. 실제 브라우저 합성 PC/390px과 지연·충돌·계정 전환 검증 완료.
- 변경: 웹13개·서버14개 코드/테스트/마이그레이션과 README3언어 및 관련 memory-bank/기술 피드 문서(총42파일). 최종 검증·미검증 범위는 docs/tech-feed/web-search-verification.md.
- 남음: 명시적 커밋·푸시·배포 승인, 운영 무료 검색 키, collector 인증/소스 승인, 실제 로그인·운영 동기화/예약 수집 검증. 이번 구현에서 운영 데이터·함수·Cron·웹은 변경하지 않았다.

## 2026-09-12 — 웹 검색 서버·화면 구현 및 작업별 검토 완료

- 구현: 공유 검색 캐시/무료 한도/RSS 독립 수집/실제 근거 요약 및 웹 관심 설정·수신 관리. 서버14개와 웹13개 코드/테스트 파일, 추가형 마이그레이션1개 포함.
- 작업별 독립 검토: 서버2차 수정, UI2차 수정 후 중요 발견 사항 모두 해결. 최종 전체 변경 검토 진행.
- 최종 통합: Node533/533, Edge10진입점 타입·8테스트, 웹 TypeScript/Vite 빌드, 모바일 호환성/타입, README24참조, diff 검사 통과. 실제 검색·운영 동기화·예약 실행은 미검증.
- 화면 검증: 합성 PC/390px, 동일 계정 충돌·다른 계정 지연/실패·수신 재개/주제 변경 겹침의 실제 화면 상태 검증. 스크린샷과 DOM 검증은 운영 검색 결과가 아니다.
- 남은 작업: 전체 검토 → 명시적 승인 후 커밋/DB/함수/웹 적용 → 앱 전용 무료 검색 키·worker 인증·소스 승인 후 실제 수집 확인. 이번 턴 운영 변경 없음.

## 2026-09-12 — 웹 검색 화면 구현 및 통합 검사 통과

- 관심 문장 입력, 수신 시작/중지/재개, 무료 한도/미연결 상태 안내, 고급 출처 설정, 실제 근거 표시와 계정 전환 요청 격리를 구현했다.
- 전체 Node530/530, 웹 타입/production 빌드 통과. 중간529/530은 신규 요청 격리 테스트의 RED 시점이었으며 수정 후 전체 재실행으로 해소했다.
- 실제 React 기반 합성 브라우저 PC/390px25개 집중 테스트 및 주요 흐름 확인. 실제 로그인/운영 수집은 미검증이다.
- 관련 파일: apps/web/src/FeedInterestSettings.tsx, TechFeedSection.tsx, techFeed 모듈/스타일/타입/테스트 및 docs/tech-feed/web-search-verification.md.
- 남은 작업: 독립 UI·최종 통합 검토 → 명시적 승인 후 커밋/운영 적용. 운영 검색 키/무료 설정과 collector 인증이 없어 현재 검색 가동 완료로 보고하지 않는다.

## 2026-09-12 — 서버 독립 검토·재검증 완료

- 서버 검토의 관심 필터와 소개 갱신/RSS 본문 소유 문제를2차 수정 후 독립 재검토 승인받았다. 실제 RSS가 본문을 채택한 경우만 보호하며 출처 연결만으로 검색 소개 갱신을 막지 않는다.
- 컨트롤러 재검증: 기술 피드87/87, Edge10진입점 검사·8/8 테스트 통과. 기존 punycode 의존성 경고는 보류하며 숨기지 않았다.
- Task1 완료, Task2 웹 화면 구현 중. HEAD7504b59와 운영 설정 유지, 커밋·푸시·배포 명시 승인 및 운영 검색 키 확인은 아직 답변 대기.

## 2026-09-12 — 관심 내용 기반 웹 검색 구현 (진행 중)

- 서버 구현/검증: 공유 검색 캐시, 무료 전용 Tavily 어댑터, owner 설정·revision, SQL lease/월간900회, 검색/RSS 실패 분리, 검색 소개의 AI 예산 연결을 로컬 구현했다. 신규 마이그레이션 `20260912122630_tech_feed_web_search.sql`.
- 검증 보고: 최종 서버 코드 전체512/512, 피드81/81, Edge10개 타입/8개 테스트 통과. 이후 테스트만1개 추가한 어댑터6/6 통과. 다음 전체 예상513이며 최종 통합에서 재검증한다.
- README3개와 PRD/설계/운영 가이드 반영 중. README 이미지24참조 검사와 문서 diff 검사 통과.
- 서버14개 파일 staged, HEAD7504b59. 커밋이 자동 승인 검토에서 차단되어 사용자에게 명시적 커밋·푸시·배포 승인을 비동기로 요청했다. 우회/재시도하지 않았다.
- 독립 서버 검토 진행 후 웹 UI 구현/브라우저 확인 예정. 운영 DB·함수·설정·배포는 아직 변경하지 않았다. 검색 키 부재로 실제 검색 가동 미확인.
- 검토 원장/보고: .superpowers/sdd/web-search-implementation/. 다음 우선순위: 서버 검토 → UI → 전체 검증 → 승인된 운영 반영.

### 2026-09-12 — 관심 입력형 피드 설계 초안

- 완료: 웹/서버/SQL 구조 점검, topic-feed-design.md 작성. 계정 연동 없는 self-service 흐름과 수집·요약의 역할 분리.
- 변경 파일: docs/tech-feed/topic-feed-design.md, memory-bank/active-context.md, progress.md, prd-tech-feed.md(검토안 연결).
- 검증: 기존 구현과 API/자료구조 대조, 문서 차이 검사. 실행 코드가 없어 테스트/빌드/배포는 하지 않음.
- 남은 작업: 기존 소스 맞춤 선별 vs 일반 웹 검색의 첫 버전 범위 확정. 이후 PRD/구현 계획과 실제 구현 진행.
...
- PR #1 을(를) main 브랜치에 squash merge했다: https://github.com/zxcc9867/studyRoom/pull/1
- 병합 커밋 SHA는 81b20025b7b833fd142c337946d885b753e7a2ba 이다.
...
- 병합 전 최신 main 기준 npm.cmd test: 338 passed
