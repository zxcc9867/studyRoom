...
## 14. 2026-08-09 Update: Domain-separated Today Workspace

### Decision

- Today 화면은 긴 단일 대시보드 대신 `집중`, `계획`, `기록`의 세 도메인 화면으로 전환한다.
- 집중에는 세션 조작, 카메라 감시, 휴식 복귀와 당일 습관 상태를 둔다.
- 계획에는 날짜별 Todo, 체크리스트와 생활계획표를 둔다.
- 기록에는 최근 7일 습관 리듬과 월별 출석 캘린더를 둔다.
- 기존 Todo·카메라·출석 데이터와 RPC는 바꾸지 않으며, 전환은 클라이언트 UI 상태로만 처리한다.

### Functional Requirements

- [x] Today 상단에 현재 도메인을 명확히 표시하는 접근 가능한 전환 버튼을 둔다.
- [x] 최근 7일 숲길 카드를 집중 세션 헤더에서 분리해 기록 화면에 단독으로 표시한다.
- [x] 계획 화면에서는 날짜 선택, 체크리스트, 생활계획표 흐름을 그대로 유지한다.
- [x] 기록 카드에서 집중 화면으로 돌아갈 수 있지만, 추가 공부 시작 CTA는 만들지 않는다.
