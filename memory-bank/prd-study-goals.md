...
## 2026-09-06 - 목표 편집창 삭제

- Today 목표 편집에서 목표 삭제 버튼을 제공한다. 기존 목표 목록 삭제와 동일한 함수를 사용한다.
- 삭제 확인에는 연결된 할 일과 공부 기록을 유지한다는 안내를 표시한다. 성공하면 목표 편집창을 닫고 목표 카드를 갱신한다.
- 완료된 목표를 편집해 저장해도 기존 완료 상태를 유지한다.

## 2026-09-06 - 목표 달성 배지

- Today 대표 목표와 목표 목록의 `목표 달성` 버튼에서 확인 후 completed 상태를 저장한다. 연결된 할 일은 자동 완료하지 않는다.
- 마이페이지는 completed 목표를 메달 배지로 표시한다. 기존 완료 목표도 포함하며 목표 ID당 하나만 표시한다. 배지는 현재 목표 상태의 표현이며 별도 영구 보상 기록이 아니다.
- 다시 진행하면 active로 돌아가 배지가 사라진다. 완료 목표 삭제도 배지를 제거하며 확인창에서 설명한다. 목표 편집은 기존 상태를 유지한다.
- 배지에는 목표명을 표시한다. 별도 완료 시각이 없으므로 created_at/updated_at을 달성일로 표시하지 않는다.
- 기존 RLS 보호 study_goals를 사용한다. DB migration/API/인증 설정 변경 없음. 목표 조회는 기존 100개 제한 대신 500개 단위 전체 페이지 조회와 고유 ID 정렬을 사용한다.
- 수용 기준: 성공 응답 후 배지 표시, 취소/실패 시 상태 유지 및 busy 해제, 새로고침 복원, 재개/삭제 반영, 390px 화면 overflow 없음.
