...
- 진행 중: 웹 production 반영과 배포 후 정시·수동 수집 완료 상태 확인. DB 마이그레이션 20260923160433 적용, Edge tech-feed v38/worker v36 ACTIVE·JWT 인증 유지. 웹은 아직 배포 전이다.
- 주의: 9월 24일 자정 직후 신규 발견 0건은 예상 가능한 결과이며, 검색 공급자가 중복 원문만 반환하면 새 목록이 늘지 않는다. RSS 추천 8곳의 이용 조건 승인 대기는 변경하지 않는다.
