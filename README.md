...
Click `새 글 확인` to collect immediately, including immediately after a completed check. Active jobs are shared; there is no five-minute cooldown. Automatic checks remain hourly. Compound interests rotate one focused search per request, plus up to four eligible RSS/API sources within the existing free limits. New articles and immediate AI summaries are not guaranteed.
Korean titles and introductions use DeepL API Free independently from AI summaries/coaching. Successful translations are shared and originals stay available in a collapsed disclosure. Translation has a separate app-wide 450,000-character UTC-month ceiling plus the provider's actual free balance; no paid fallback. Missing key, quota exhaustion or failure keeps the original visible. An operator-provided Free key is required; readers do not need one. [Translation setup and verification](docs/tech-feed/korean-translation.md).
