...
alter table public.notification_targets
  add constraint notification_targets_kind_check check (kind in ('expo', 'web_push', 'email', 'kakao_memo', 'telegram'));
...
alter table public.notification_targets
  add constraint notification_targets_payload_check check (
    (kind in ('expo', 'email', 'kakao_memo', 'telegram') and destination is not null)
...
alter table public.notification_deliveries
  add constraint notification_deliveries_channel_check check (channel in ('expo', 'web_push', 'email', 'kakao_memo', 'telegram'));
