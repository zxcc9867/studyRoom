...
async function recommendationReady(admin: Admin, uid: string, rec: any, settings: any, zone: string, kind: string) {
    const now = Date.now();
    if (!eligible(uid) || !settings.enabled || rec.status !== 'pending' || Date.parse(rec.start_at) < now || Date.parse(settings.muted_until) > now)
        return false;
    if (kind === 'opportunity') {
        ...
        if (connected.length) {
            try {
                await syncGoogle(admin, uid);
            }
            catch {
                return false;
            }
        }
        const recent = unwrap(await admin.from('notification_deliveries').select('id').eq('user_id', uid).eq('status', 'sent').gte('created_at', new Date(now - 15 * 60000).toISOString()).limit(1));
        if (recent.length)
            return false;
    }
    const events = unwrap(await admin.from('coach_events').select('*').eq('user_id', uid).limit(1000));
    const todos = unwrap(await admin.from('study_todos').select('local_date,start_time,end_time,is_completed').eq('user_id', uid).gte('local_date', addDays(rec.local_date, -1)).lte('local_date', rec.local_date).limit(1000));
    if (events.length >= 1000 || todos.length >= 1000)
        return false;
    return freeSlots({ day: rec.local_date, zone, settings, events, todos, now }).some((slot: any) => Date.parse(slot.start_at) <= Date.parse(rec.start_at) && Date.parse(slot.end_at) >= Date.parse(rec.end_at));
}
export async function sendCoachTarget(target: any, recommendation: any, deliveryId: string, env = envDefault) {
    ...
    if (target.kind === 'slack') {
        const token = env('SLACK_BOT_TOKEN') || env('STUDY_ALERT_SLACK_BOT_TOKEN');
        if (!token)
        ...
        if (!pushEndpointAllowed(target.subscription))
            throw new Error('invalid_push_target');
        ...
        await webpush.sendNotification(target.subscription, JSON.stringify({ title, body, url: '/#today', tag: deliveryId }), { TTL: 600, timeout: 10000 });
        return;
    }
    if (!response.ok)
        throw new Error(response.status >= 500 ? 'provider_uncertain' : 'provider_rejected');
}
export async function dispatchCoaching(admin: Admin, userId?: string) {
    let query = admin.from('coach_settings').select('*').eq('enabled', true);
    if (userId)
        query = query.eq('user_id', userId);
    const settings = unwrap(await query.limit(100));
    let sent = 0;
    for (const initial of settings) {
        ...
        for (const delivery of pending) {
            ...
            const claimed = unwrap(await admin.from('coach_deliveries').update({ status: 'sending' }).eq('id', delivery.id).eq('status', 'pending').select().maybeSingle());
            if (!claimed)
                continue;
            try {
                await sendCoachTarget(target, rec, delivery.id);
                unwrap(await admin.from('coach_deliveries').update({ status: 'sent', sent_at: new Date().toISOString() }).eq('id', delivery.id));
                sent++;
            }
            catch {
                await admin.from('coach_deliveries').update({ status: 'unknown' }).eq('id', delivery.id);
            }
        }
        ...
        const rec = recommendations.find((r: any) => dueKind(initial, r, now, zone));
        if (!rec)
            continue;
        const kind = dueKind(initial, rec, now, zone);
        const active = unwrap(await admin.from('study_sessions').select('id').eq('user_id', uid).eq('status', 'active').limit(1));
        if (active.length)
            continue;
        ...
        for (const target of eligibleTargets(initial, targets, profile)) {
            ...
            if (!currentTarget || !eligibleTargets(current, [currentTarget], profile).length || !dueKind(current, { ...rec, ...currentRec }, new Date(), zone) || !await recommendationReady(admin, uid, { ...rec, ...currentRec }, current, zone, kind!)) {
                await admin.from('coach_deliveries').update({ status: 'cancelled' }).eq('id', delivery.id);
                continue;
            }
            const claim = unwrap(await admin.from('coach_deliveries').update({ status: 'sending' }).eq('id', delivery.id).eq('status', 'pending').select().maybeSingle());
            if (!claim)
                continue;
            try {
                await sendCoachTarget(currentTarget, rec, delivery.id);
                unwrap(await admin.from('coach_deliveries').update({ status: 'sent', sent_at: new Date().toISOString() }).eq('id', delivery.id));
                sent++;
            }
            catch (error) {
                const known = ['not_configured', 'invalid_push_target', 'provider_rejected'].includes((error as Error).message);
                await admin.from('coach_deliveries').update({ status: known ? 'failed' : 'unknown' }).eq('id', delivery.id);
            }
        }
    }
    return { sent };
...
export async function handleCoachSlackAction(admin: Admin, userId: string, actionId: string, value: string) {
    if (!['coach_snooze', 'coach_mute'].includes(actionId))
        return false;
    if (actionId === 'coach_snooze' && !eligible(userId))
        throw new Error('coach_disabled');
    const delivery = unwrap(await admin.from('coach_deliveries').select('*').eq('id', value).eq('user_id', userId).eq('channel', 'slack').maybeSingle());
    if (!delivery)
        throw new Error('not_authorized');
    if (actionId === 'coach_mute') {
        const profile = unwrap(await admin.from('profiles').select('time_zone').eq('user_id', userId).single());
        const now = new Date(), date = localParts(now, profile.time_zone).date;
        let end = new Date(now.getTime() + 60000);
        while (localParts(end, profile.time_zone).date === date)
