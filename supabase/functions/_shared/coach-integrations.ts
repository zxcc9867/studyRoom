...
export async function syncGoogle(admin: Admin, userId: string, env: Env = envDefault) {
    ...
    try {
        ...
        const token = await googleToken(admin, row, env);
        const events = [];
        for (const id of (row.config?.calendar_ids || [])) {
            ...
            for (const event of await googlePages(url, token)) {
        ...
        unwrap(await admin.rpc('coach_google_snapshot', { p_user_id: userId, p_connection_id: row.id, p_events: events, p_expected_config: row.config }));
...
export async function analyzeRepositories(admin: Admin, userId: string, env: Env = envDefault) {
    ...
    for (const repo of repositories) {
        ...
        if (!verified) {
            await admin.from('coach_repositories').update({ analysis: [], last_checked_at: new Date().toISOString() }).eq('id', repo.id);
            continue;
        }
        ...
        if (fresh.id !== row.id)
            fail('connection_changed');
        unwrap(await admin.rpc('coach_repository_result', { p_user_id: userId, p_repo_id: repo.id, p_connection_id: row.id, p_expected_config: row.config, p_expected_ai_enabled: repo.ai_enabled, p_sha: commit.sha, p_analysis: analysis }));
    }
