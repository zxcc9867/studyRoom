...
Deno.serve(async request=>{
  ...
  try {
    ...
    if(secret&&request.headers.get('x-cron-secret')===secret) return json(await dispatchCoaching(admin));
    const {data:{user},error}=await admin.auth.getUser((request.headers.get('authorization')||'').replace(/^Bearer\s+/i,''));
    if(error||!user||user.is_anonymous) return json({error:'Unauthorized'},401);
    const body=await request.json();if(body.action!=='test')return json({error:'invalid_action'},400);
    ...
    const target=eligibleTargets(settings,targets,profile)[0];if(!target)return json({error:'연결하고 활성화한 알림 채널이 필요합니다.'},400);
    await sendCoachTarget(target,{title:'스터디룸 코칭 알림 테스트',duration_minutes:15},crypto.randomUUID());return json({ok:true});
  } catch {return json({error:'알림을 완료하지 못했습니다. 연결 설정을 확인하세요.'},503);}
});
