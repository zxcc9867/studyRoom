...
6. Scheduled cache TTL stays one hour. Approved 2026-09-13 immediate amendment: explicit authenticated refresh bypasses account/topic/source freshness with no five-minute cooldown. Active leases coalesce concurrent work; provider failure backoff/free-only validation/monthly cap remain. One search and up to four approved subscribed sources per request. Compound interests separated by comma/semicolon/및/그리고/and rotate one focused query per reserved search attempt (including failure); canonical identity and private saved input remain unchanged. Single topics retain their query. No guarantee of new articles on every click.
...
8. User-approved nullable-cap amendment (2026-09-13): GET /usage requires Free/Researcher, finite nonnegative plan/key/paygo usage, 0<plan_limit<=1000, paygo_usage=0. Explicit null key.limit uses remaining plan credits; numeric key.limit must be 0..1000 and further constrains remaining credits. paygo_limit may be exactly null or0, never a positive value. Null does not prove billing is disabled. Missing fields, malformed types, paid/unknown plans and unavailable usage fail closed. Less than one remaining credit stops search. Keep the dedicated no-card account, provider lease, basic-only request and atomic app900 cap; never enable billing or paid fallback.
...
- refresh {expected_revision:number} => state started internally; public result ready/partial/running/deferred/paused/not_configured/no_sources/unavailable, optional rss/search outcomes (legacy cooldown/retry_after responses remain understood during rollout). Account identity comes only from authentication. Current preference revision is revalidated under the owner lock when claiming work.
