...
- Actions: `start`, `resume`, `switch`. Start selects owned incomplete todos scheduled today; resume/switch select existing linked, incomplete todos (including previous/next dates). Exactly one current todo must belong to the selection. Single selection can be chosen by the UI automatically.
...
- Preview blocking errors: `INVALID_ACTION`, `CURRENT_TODO_REQUIRED`, `INVALID_CURRENT_TODO`, `ACTIVE_SESSION_EXISTS`, `ACTIVE_SESSION_NOT_FOUND`, `LEASE_EXPIRED`, `SESSION_NOT_PAUSED`, `SESSION_PAUSED`, `ALREADY_CURRENT_TODO`, `RECOVERY_REQUIRED`, `INVALID_SELECTION`, `INVALID_EXCLUSION`, `CASCADE_LIMIT`.
