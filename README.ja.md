...
収集設定と閲覧フィルターを分けます。自由入力の興味は**何を収集するか**、折りたたみ式のトピック・配信元フィルターは**収集済みの何を見るか**を決めます。最新・保存の両方で現在のページだけでなく全対象記事から選択肢を作り、長い興味の文の代わりに本文に根拠のある短いタグを表示します。

AI分類を優先し、ない場合は明確な根拠がある記事だけを技術ニュース・実務チュートリアル・事例と深掘りに補完分類します。無理な分類や未分類ラベルの反復は避けます。紹介のプレビューからMarkdown記号を取り除き、展開時は見出し・リスト・コードを安全に表示します。外部HTMLは実行しません。

`오늘 수집된 내 피드`はプロフィールのタイムゾーンと記事の初回収集時刻で1日全体を自動集計します。公開日・現在のページ・フィルターとは独立しています。AIインサイトはボタン操作時だけ、権限を確認した最大24件の紹介を分析し、原文への根拠リンクを示します。十分な紹介が最低2件必要で、動画の内容は分析しません。キャッシュを再利用し、新着時は更新が必要と表示、アクセス権を失った結果は非表示にします。既存の実呼び出し6回/ユーザー/日を共有し失敗も数え、有料には切り替えません。[設計と制限](docs/tech-feed/daily-briefing-design.md) · [反映状況](memory-bank/progress.md)。

原文が提供するサムネイルをカード内に表示し、図を切り取らず比率を保ちます。添付YouTube/Vimeo動画はクリック後に読み込み、自動再生せず閉じられます。画像なし・アクセス制限・失敗時も本文と出典リンクを保持します。過去の記事も手動・定期処理ごとに最大3件ずつ共有キャッシュへ反映するため、全記事のメディアは保証しません。無関係な画像検索・全文保存・追加AI/有料APIは利用しません。
