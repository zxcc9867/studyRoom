...
Webで公開技術への興味を入力し、受信・変更・一時停止できます。一般ユーザーの検索APIキーやSNS連携は不要です。新技術の発見 → 韓国語要約または配信元・検索結果の紹介 → 原文 → 学習タスクへつなげます。最新/保存、フィルター、20件ずつの追加読み込みを提供し、配信元の購読は詳細設定にまとめます。開始画面は引き続き「今日」、設定・保存・タスクの関連付けはユーザー別にサーバーへ保存します。
...
`새 글 확인`を押すと収集をすぐにリクエストし、終了後に一覧を更新します。進行中の処理を共有し、アカウント・テーマ・配信元ごとに5分の制限を適用します。定期収集は1時間基準のままです。1回に1テーマと最大4つの対象RSS/APIを無料枠内で確認し、新着記事や即時AI要約を保証するものではありません。
