...
**コードは本番にデプロイ済みですが、フィードは無効です。** DB・関数・Web の反映と無効な Cron の登録を完了しました。利用条件の承認、無料 AI 設定、パイロット・実環境の検証後に有効化します。[デプロイ記録](docs/tech-feed/deployment-20260912.md) · [要件](memory-bank/prd-tech-feed.md) · [リリース手順](docs/tech-feed/release.md) · [キャリア機能の復元](archive/career-coach/README.md)。タイムゾーン設定と学習再開コーチングは維持します。
